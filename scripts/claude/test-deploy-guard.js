...
const CASES = [
  ...
  ["Bash", "ssh -o BatchMode=yes root@45.77.198.133 'bash /srv/gestorpro/deploy/limpiar-backups.sh --dry-run'", false, "whitelist: limpiar-backups.sh via ssh con --dry-run (permitido)"],
  ["Bash", "ssh -o BatchMode=yes root@45.77.198.133 'bash /srv/gestorpro/deploy/limpiar-backups.sh --dias=30 --conservar=5'", false, "whitelist: limpiar-backups.sh via ssh con flags numericos (permitido)"],
  ["Bash", 'ssh root@45.77.198.133 "bash /srv/gestorpro/deploy/limpiar-backups.sh"', false, "whitelist: limpiar-backups.sh via ssh sin flags, comillas dobles (permitido)"],
  ...
  ["Bash", "ssh root@45.77.198.133 'bash /srv/gestorpro/deploy/limpiar-backups.sh; rm -rf /srv/gestorpro/deploy/backups'", true, "whitelist NO cubre encadenar: ; rm deploy/backups sigue bloqueado"],
  ["Bash", "ssh root@45.77.198.133 'bash /srv/gestorpro/deploy/limpiar-backups.sh && rm /srv/gestorpro/deploy/backups/x.dump'", true, "whitelist NO cubre encadenar: && rm sigue bloqueado"],
  ...
  ["Bash", "ssh root@45.77.198.133 'rm /srv/gestorpro/deploy/backups/x.dump'", true, "P0-BACKUP intacto: rm via ssh sigue bloqueado"],
  ...
  ["Bash", "ssh $(rm -rf /srv/gestorpro/deploy/backups) root@45.77.198.133 'bash /srv/gestorpro/deploy/limpiar-backups.sh --dry-run'", true, "whitelist NO cubre $() en el tramo ssh: rm dentro de la sustitucion bloqueado"],
  ["Bash", "ssh `rm -rf /srv/gestorpro/deploy/backups` root@45.77.198.133 'bash /srv/gestorpro/deploy/limpiar-backups.sh'", true, "whitelist NO cubre backticks en el tramo ssh (normalize los quita, el rm queda visible)"],
  ["Bash", "ssh -oProxyCommand=rm$IFS-rf$IFS/srv/gestorpro/deploy/backups root@45.77.198.133 'bash /srv/gestorpro/deploy/limpiar-backups.sh'", true, "whitelist NO cubre -oProxyCommand (opcion fuera de la lista cerrada)"],
  ["Bash", "ssh root@45.77.198.133 'bash /srv/gestorpro/deploy/limpiar-backups.sh\" && rm /srv/gestorpro/deploy/backups/x.dump'", true, "comillas de apertura/cierre desparejadas no matchean la whitelist (backreference)"],
  ["Bash", "ssh root@1.2.3.4 'bash /srv/gestorpro/deploy/limpiar-backups.sh; rm -rf /srv/gestorpro/deploy/backups'", true, "host distinto del VPS + encadenado: bloqueado"],
