...
**GestorPro** (`/root/gestorpro`, compose `deploy`: caddy con 80/443 + backend +
...
1. **Backup normal** de GestorPro (`bash /root/gestorpro/deploy/backup.sh`, verificar
...
7. **Post-check** (todo obligatorio):
   ...
   cd /root/gestorpro && git rev-parse HEAD             # = local = origin
   ```
...
cd /root/gestorpro && git checkout 3527120 -- deploy/       # versión pre-borde
...
## Fase C (otro día, proyecto WinFleet) — ordenar y limpiar

No es de GestorPro; se anota para que no se olvide.
- Mover `/opt/winfleet` (releases), `/var/lib/winfleet` (datos), `/etc/winfleet`
  (env) bajo `/srv/winfleet/`; recrear el contenedor con las rutas nuevas y
  `NETWORK=edge` como valor por defecto de `upgrade-vps.sh` (cambio en el repo
  WinFleet).
- Borrar contenedores parados `winfleet-backup-*` / `winfleet-rollback-*` (13) e
  imágenes viejas (dejar la actual y la anterior); `/opt/winfleet-05e875d`,
  `/opt/winfleet-12293d9`, `/root/deploy_*.log`.
- Mover `/root/gestorpro` → `/srv/gestorpro` (cambiar rutas en `/root/ops/*.sh`,
  `deploy/offsite-diario.sh`, la whitelist del guard `.claude/hooks/deploy-guard.js`
  y el crontab). Es un `mv` de directorio: el proyecto compose sigue siendo `deploy`
  (`name:` explícito), así que volúmenes y red no cambian.
